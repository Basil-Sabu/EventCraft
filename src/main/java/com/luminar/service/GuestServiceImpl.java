...
public class GuestServiceImpl implements GuestService {
    ...
    public void saveGuest(Guest guest) {
        ...
        if (guest.getGroupId() == null) {
            ...
            boolean exists =
                guestRepository
                    .existsByEventEventIdAndGroupNameIgnoreCase(
                        eventId,
                        guest.getGroupName()
                    );
            ...
            if (exists) {
                throw new RuntimeException(
                    "Guest group with the same name already exists for this event"
                );
            }
        }
        // UPDATE case
            ...
            boolean exists =
                guestRepository
                    .existsByEventEventIdAndGroupNameIgnoreCaseAndGroupIdNot(
                        eventId,
                        guest.getGroupName(),
                        guest.getGroupId()
                    );
            ...
            if (exists) {
                throw new RuntimeException(
                    "Another guest group with the same name already exists for this event"
                );
            }
        }
    ...
    public Guest getGuestById(Long guestId) {
        return guestRepository.findById(guestId)
                .orElseThrow(() ->
                    new RuntimeException("Guest group not found")
                );
    }
